...
# upload everything in src/web to demo website, using $PASSWORD arg presence as switch
if [ ! -z $PASSWORD ]; then
    docker run \
        -e $HOST \
        -e $USERNAME \
...
if [ ! -z $GH_TOKEN ]; then
    ...
    curl --data-binary @"./src/web/js/bootstrip.min.js" -H "Authorization: token $GH_TOKEN" -H "Content-Type: application/octet-stream" https://uploads.github.com/repos/$repo/releases/$id/assets?name=bootstrip.min.js
    ...
    curl --data-binary @"./src/web/css/bootstrip.min.css" -H "Authorization: token $GH_TOKEN" -H "Content-Type: application/octet-stream" https://uploads.github.com/repos/$repo/releases/$id/assets?name=bootstrip.min.css
    ...
    curl --data-binary @"./src/web/css/bootstrip-theme-default.min.css" -H "Authorization: token $GH_TOKEN" -H "Content-Type: application/octet-stream" https://uploads.github.com/repos/$repo/releases/$id/assets?name=bootstrip-theme-default.min.css
    ...
    curl --data-binary @"./src/web/css/bootstrip-theme-darkmoon.min.css" -H "Authorization: token $GH_TOKEN" -H "Content-Type: application/octet-stream" https://uploads.github.com/repos/$repo/releases/$id/assets?name=bootstrip-theme-darkmoon.min.css
